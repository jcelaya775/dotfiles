...
function c..() {
  cd ..
  dir="$(fd --type directory -H --max-depth 1 .. | fzf)"
  cd "$dir" || exit
}
...
function vp() {
  ...
  dir="$HOME/repos/$(fd --type directory --max-depth 1 | fzf)"
  cd "$dir" || exit
  if [[ "$1" != "" ]]; then
    num_tabs=$(( $1 - 1 ))
    nt "$num_tabs"
...
__conda_setup="$('/home/jorge/anaconda3/bin/conda' 'shell.bash' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
